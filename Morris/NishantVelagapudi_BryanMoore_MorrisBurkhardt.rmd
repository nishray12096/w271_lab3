...
ts_mts_test = ts(ts_mts[time(ts_mts) >= 1993, ], start = c(1993, 1), freq = 12)
...
All the time series seem to have a common deterministic trend. The stochastic trend also seems to be similar between the four time series - just different in magnitude, with series1 having the strongest stochastic trend, followed by series4, series3 and then series2.

Let us run a Dickey-Fuller Tests to test for unit roots.

```{r}
adf.test(df_mts_train$series1)
adf.test(df_mts_train$series2)
adf.test(df_mts_train$series3)
adf.test(df_mts_train$series4)
```
...
==> Only series4 is likely to be stationary. All the other series are likely to contain unit roots!!
...
In a few cases we reject the null hypothesis. In those cases, it is likely that our time series are cointegrated. This means, we would be able to fit a linear regression between both time series... ?!
