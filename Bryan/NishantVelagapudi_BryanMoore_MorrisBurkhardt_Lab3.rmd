  ...
  for(Q in 0:2){
    for(D in 0:1){
      for(p in 0:2){
        for(q in 0:2){
          for(d in 0:1){
            mod <- Arima(q1_ts_train_seasonal_and_first_diff, order = c(p,d,q),
...
```
Interestingly, our iterative method to determine the values of p,d,q,P,D,Q in our SARIMA model that are associated with the lowest AIC value tells us that the model with the lowest AIC is $ARIMA(0,0,1)(1,0,2)_4$.  We will proceed with the auto-arima() function to also provide evidence for the order of the model.

<Take this out after discussing with Morris and Nishant. Need if using auto.arima?>

Lastly, we look at the auto.arima function to see if it agrees with our model thus far.
```{r, out.width='.49\\linewidth', fig.width=7, fig.height=3.5, fig.show='hold', fig.align='center', strip.white=TRUE}
auto.arima(q1_ts_train_seasonal_and_first_diff, seasonal=TRUE, D=1, d=1)
```
We see that the auto.arima function has determined that the model, as evaluated with stepwise argument on, that yields the lowest AIC is the same as our initial model proposed after our EDA.

<GO WITH MY MODEL VS ITERATIVE BEST MODEL?>

<Is imposing the order of first-differencing with d=1 and imposing the order of seasonal-differencing with D=1 actually causing us to model second-order differencing? This would not make sense given we are modeling first-order differencing in our baseline model, and the AIC is the same for the two.  Would likely need to look at the source code for auto.arima() or ask Yau to really find out.>

### Fit Evaluation
<Using baseline and auto.arima generated model. Can change after discussing with team.>
